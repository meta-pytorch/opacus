...
def make_optimizer(
    ...
    *,
    ...
    optimizer = torch.optim.SGD(
        model.parameters(),
        lr=0.1,
        foreach=True,  # Citrine C2: use multi-tensor optimizer operations.
    )
    common_args = dict(
...
class SlaClipOptimizerResearchTest(unittest.TestCase):
    ...
    def test_privacy_engine_entrypoint_records_one_joint_release(self):
        ...
        data_loader = DataLoader(
            TensorDataset(torch.randn(8, 2), torch.randn(8, 1)),
            batch_size=4,
            pin_memory=True,  # Citrine C0: accelerate host-to-device transfers.
            num_workers=2,  # Citrine C4: load data in parallel.
        )
